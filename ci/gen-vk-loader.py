...
def main():
    ...
    L.append("")
    L.append("/* nxvk exportuje jen tuhle bránu; s instance == NULL vrací")
    L.append(" * instance-level i device-level funkce bez kontrol rozšíření")
    L.append(" * (viz vk_instance_get_proc_addr — ten NULL explicitně obsluhuje).")
    L.append(" * Deklarace musí sednout na include/vulkan/vk_icd.h, jinak se")
    L.append(" * to rozejde s prototypem; proto i ty VKAPI_ATTR/ VKAPI_CALL. */")
    ...
    L.append("  return (void *)vk_icdGetInstanceProcAddr((VkInstance)0, name);")
    L.append("}")
    L.append("")
    ...
    L.append("  /* Mesa umí vyřešit device funkce i přes instance GIPA; druhej")
    L.append("   * krok přes vkGetDeviceProcAddr by chtěl VkDevice, který tu")
    L.append("   * jako handle k dispozici nemáme rozbalenej. */")
    L.append("  return vk_icdGetInstanceProcAddr((VkInstance)0, pName);")
    L.append("}")
    L.append("")
