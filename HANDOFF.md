...
| aktuální build | **55** (binárka; `NSX_CI_BUILD=55` v `ci/patches/ci_core_log.c`). Poslední úspěšný run **`35360645184`** (commit `4dd9fcb`, 2026-09-18 15:10 UTC): `NetherSX2.nro` = **71 602 311 B**, `sha256=1ef0d66c85a3d7b0`, uvnitř `NetherSX2_nx_vk.nro` = 23 108 483 B, upstream `f084dc1`, Mesa SDK z runu (job `mesa` skipped → běh ~2 min). Tři běhy buildu 55 (`35358359052`/`b98a772` `sha256=ec0880d0488379e8`, `35359982523`/`a0d34d5` `sha256=15256d1c0ed0142e`, `35360645184`/`4dd9fcb` `sha256=1ef0d66c85a3d7b0`) mají **identickou velikost** — liší se jen otisk (build není bitově deterministický), proto **rozlišuj podle `sha256`**, ne podle velikosti. **Jméno release** bylo dřív „CI build NN" = `GITHUB_RUN_NUMBER` (a přestalo sedět: release říkala 56/57, binárka byla 55). Od commitu, který přidává `out/ci-build.txt`, je název „binarka build NN, run MM". Binárku na kartě poznáš podle `[CI] session start build=55`. Předchozí: 54 = 71 598 215 B (run `35348818842`), 51 = 71 594 119 B, `sha256=9c719e25cb27b374` |
