...
| aktuální build | **55** — CI run **`35358359052`** (success, 2026-09-18 14:48 UTC, branch `arena/01a0b4ce-test`, commit `b98a772`), `NetherSX2.nro` = **71 602 311 B**, `sha256=ec0880d0488379e8`, uvnitř `NetherSX2_nx_vk.nro` = 23 108 483 B, upstream `f084dc1`, Mesa SDK z runu (job `mesa` skipped). **Pozor na jméno release:** „NetherSX2.nro (CI build 55, Vulkan)" je `GITHUB_RUN_NUMBER` (pořádí runu v repu), **ne** `NSX_CI_BUILD` z `ci_core_log.c` — shodují se náhodou. Binárku na kartě poznáš podle `[CI] session start build=55`. Předchozí: 54 = 71 598 215 B (run `35348818842`), 51 = 71 594 119 B, `sha256=9c719e25cb27b374` |
