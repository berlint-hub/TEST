...
| aktuální build | **55** (binárka; `NSX_CI_BUILD=55`). Release `nro-latest` = poslední CI run **`35359982523`** (success, 2026-09-18 15:04 UTC, commit `a0d34d5`), `NetherSX2.nro` = **71 602 311 B**, `sha256=15256d1c0ed0142e`; první běh buildu 55 byl run `35358359052` (commit `b98a772`, `sha256=ec0880d0488379e8`, **stejná velikost** — liší se jen otisk). Uvnitř `NetherSX2_nx_vk.nro` = 23 108 483 B, upstream `f084dc1`, Mesa SDK z runu (job `mesa` skipped → běh ~2 min). **Pozor na jméno release:** „NetherSX2.nro (CI build **56**, Vulkan)" je `GITHUB_RUN_NUMBER` (pořadí runu v repu), **ne** `NSX_CI_BUILD` — ta čísla se už **neshodují**. Binárku na kartě poznáš jedině podle `[CI] session start build=55`. Předchozí: 54 = 71 598 215 B (run `35348818842`), 51 = 71 594 119 B, `sha256=9c719e25cb27b374` |
